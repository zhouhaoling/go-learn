...
func Marshal(data interface{}) (result []byte, err error) {
	...
	for i := 0; i < typeInfo.NumField(); i++ {
		...
		//得到的是mysql、server的结构体,以及它们里面的值
		//{ServerConf  main.ServerConfig ini:"server" 0 [0] false} {127.0.0.1 8000}
		//{MysqlConf  main.MysqlConfig ini:"mysql" 24 [1] false} {    9000 0}
		fmt.Println(typeInfo.Field(i), valueInfo.Field(i))
